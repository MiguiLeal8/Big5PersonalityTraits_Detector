...
def calcular_big5(emociones_resultados):
    big5_scores = {trait: 0 for trait in big5_mapeo.keys()}
    
    # Ponderación basada en la probabilidad de cada emoción
    for trait, emociones in big5_mapeo.items():
...
def escalar_puntajes(go_emotions_puntajes, bfi_rangos):
    bfi_puntajes_escalados = {}

    for rasgo, valor in go_emotions_puntajes.items():
        min_bfi, max_bfi = bfi_rangos[rasgo]
...
def actualizar_big5(previos, nuevos, alpha):
